...
public class NegocioSecretaria implements INegocioSecretaria {
	...
	public void insert(Secretaria secretaria) throws Exception {
		...
		/*
		 * if (!EmailValidator.getInstance().isValid(secretaria.getEmail())) { throw new
		 * Exception("E-Mail inválido"); }
		 */

		if (secretaria.getCpf().trim().equals("   .   .   -  ") || (secretaria.getCpf().isEmpty())) {
		...
		if (secretaria.getRg().length() > RG_SIZE) {
		...
		/*
		 * if (!ValidatorUtils.validate(secretaria.getDtNascimento())) { throw new
		...
		if (secretaria.getEndereco().getCep().trim().equals("  .   -   ")
				|| (secretaria.getEndereco().getCep().isEmpty())) {
